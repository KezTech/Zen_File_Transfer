...
import 'package:zen_file_transfer/models/file_model.dart';
...
class Sender {
  ...
  static _startServer(List<String?> fileList, BuildContext context) async {
    ...
    //check if no imp\proper address is assigned
    ...
    bool? allowRequest;
    //listen requestf from receiver
    _server.listen(
      (HttpRequest request) async {
        if (request.requestedUri.toString() ==
            'http://$_address:4040/server') {
...
//uri should be in format http://ip:port/secrecode/file-index
          ...
          if (int.parse(requriToList[requriToList.length - 2]) ==
            ...
            request.response
                .write('Wrong secret-code.server denied access');
            request.response.close();
